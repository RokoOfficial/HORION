...
if __name__ == "__main__":
    ...
    def analyze_texts_example(prefix: str = "OUT", data: list = None) -> List[str]:
    ...
    analyzed = analyze_texts_example(data=texts_data, prefix="OUT")
...
"""
ROKO Pipeline Autônomo — versão AutoFlux
========================================
• Paralelismo agora é feito com AutoFlux (https://github.com/RokoOfficial/autoflux).
• Mantém efeito de escrita ao vivo, correções automáticas e streaming de saída.
• Requer:  pip install git+https://github.com/RokoOfficial/autoflux
"""
...
# AutoFlux pode não estar instalado na primeira execução.
try:
    from autoflux import AutoFlux
except ModuleNotFoundError as e:  # orienta o usuário e encerra
    raise RuntimeError(
        "AutoFlux não encontrado. Instale com:\n"
        "pip install git+https://github.com/RokoOfficial/autoflux"
    ) from e
...
def type_effect(text: str, delay: float = 0.001) -> None:
    for char in text:
...
# ... [código completo continua abaixo no próximo comando] ...

# ---------------------------------------------------------------------
# AGENTES INDIVIDUAIS (continuação)
# ---------------------------------------------------------------------

# ...demais classes e código finalizado anteriormente...

# ---------------------------------------------------------------------
# TESTE INTERATIVO
# ---------------------------------------------------------------------
if __name__ == "__main__":
    roko = ROKO(max_main_cores=3, max_sub_cores=6)
    print("\nROKO pronto! Digite um pedido (ou 'sair').")
    while True:
        try:
            pedido = input("\nUsuário> ").strip()
        except (EOFError, KeyboardInterrupt):
            print("\nROKO desligando. Até logo!")
            break
        if pedido.lower() in {"sair", "exit", "quit"}:
            print("ROKO desligando. Até logo!")
            break
        for parte in roko.run(pedido):
            type_effect(parte, delay=0.02)

# Adicionando definição da classe ROKO...
